...
export const AdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingFrame) {
        await axios.put(
          `${BACKEND_URL}/api/admin/frames/${editingFrame.id}`,
          formData,
  ...
  const handleDelete = async (frameId) => {
    ...
    try {
      await axios.delete(
        `${BACKEND_URL}/api/admin/frames/${frameId}`,
        { headers: getAuthHeader() }
      );
          ...
          {frames.map((frame) => (
            <div key={frame.id} className="bg-white rounded-lg shadow-lg overflow-hidden">
              ...
              <div className="p-4">
                ...
                <p className="text-blue-600 font-bold text-xl mb-2">₹{frame.price}</p>
                <p className="text-sm text-gray-500 mb-4 capitalize">{frame.category.replace('_', ' ')}</p>
